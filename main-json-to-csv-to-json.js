...
function readJsonSrc(json) {
    ...
    document.getElementById('btnDownloadCsv').toggleAttribute('disabled');
...
function downloadJsonCsv() {
    const filename = jsonFileName.split('.json')[0].concat('.csv');
    downloadFile(new File([jsonCsv], filename, {type: 'text/csv'}));
}
...
jsonToArray = function (json) {
    ...
    const jsonObj = JSON.parse(json).openmct;
    console.log('jsonToArray', jsonObj);
    ...
    for (let key in jsonObj) {
        const objArr = [
            ...
            jsonObj[key].name,
            key,
            jsonObj[key].location,
            JSON.stringify(jsonObj[key].composition),
            JSON.stringify(jsonObj[key].configuration)
        ]
        jsonArray.push(objArr);
    }

    console.log(jsonArray);
    jsonCsv = arrayToCSV(jsonArray);
}
