...
function createOpenMCTJSONfromCSV(csv) {
    ...
    for (let i = 0; i < plotObjectArr.length; i++) {
        // First look at OverlayPlotName
        const telemObj = plotObjectArr[i];
        const telemObjDataSrc = telemObj.DataSourceFullPath;
        ...
        if (overlayPlotName.length > 0) {
            ...
            curOverlayPlotObj.addToComposition(telemObjDataSrc, getNamespace(telemObjDataSrc));
        } else {
            stackedPlotObj.addToComposition(telemObjDataSrc, getNamespace(telemObjDataSrc));
        }

        // console.log(plotObjectArr[i].OverlayPlotName);
    }

    console.log(stackedPlotObj);

}

function createOpenMCTJSONfromCSVOLD(csv) {
    /*
